...
function rememberSlot(year, player) {
  ...
  slotOverrides[year][player.id] = { position: player.position, status: player.status };
}
function overrideFor(id, year) {
  let best = null;
  Object.keys(slotOverrides).map(Number).sort().forEach(y => {
    if (y <= year && slotOverrides[y] && slotOverrides[y][id]) best = slotOverrides[y][id];
  });
  return best;
}
function applySlotOverrides(years) {
  Object.keys(years).map(Number).sort().forEach(y => {
    years[y].players.forEach(p => {
      const ov = overrideFor(p.id, y);
      if (!ov) return;
      let status = ov.status;
      const c = (p.contract || "").toUpperCase();
      if (status === "MINORS" && c === "R2") status = "RESERVE";
      p.status = status;
      if (status === "ACTIVE" && ov.position) p.position = ov.position;
    });
  });
}
function buildYears() {
      ...
      if (targetStatus === "ACTIVE" && !["BENCH","IR","MINORS"].includes(targetPos)) {
        ...
        if (occupants.length >= max) {
          ...
          if (inSlot) {
            ...
            rememberSlot(currentYear, inSlot);
          }
        }
        p.status = "ACTIVE";
